...
def validate_email_local_part(local: str, allow_smtputf8: bool = True, allow_empty_local: bool = False,
    ...
    # restrictions on the placement of dots, as in dot-atom local parts.
        ...
        if bad_chars:
            raise EmailSyntaxError(
                'The email address contains invalid characters in quotes before the @-sign: ' + ', '.join(sorted(bad_chars)) + '.')
    ...
    # (RFC 5322 3.2.3)
    check_dot_atom(local, 'An email address cannot start with a {}.',
                   'An email address cannot have a {} immediately before the @-sign.', is_hostname=False)
...
def validate_email_domain_name(domain, test_environment=False, globally_deliverable=True):
    ...
    if bad_chars:
        raise EmailSyntaxError('The part after the @-sign contains invalid characters: ' +
                               ', '.join(sorted(bad_chars)) + '.')
    ...
    check_dot_atom(domain, 'An email address cannot have a {} immediately after the @-sign.',
                   'An email address cannot end with a {}.', is_hostname=True)
...
def validate_email_domain_literal(domain_literal, allow_domain_literal=False):
    ...
    if bad_chars:
        raise EmailSyntaxError('The part after the @-sign contains invalid characters in brackets: ' +
                               ', '.join(sorted(bad_chars)) + '.')
