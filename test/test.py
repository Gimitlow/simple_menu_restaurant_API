...
def test_create_menu():
    ...
    assert response.json()['description'] in params.params_menu_create['description']
...
def test_get_all_menus_2():
    ...
    for row in response.json():
        assert row['id'] and row['title'] and row['description'] and row['submenus_count'] != [] and row['dishes_count'] != []

@pytest.mark.menu
def test_get_target_menu():
    response = requests.get(f'{base_url}/menus/{params.menu_id}')
    assert response.status_code == 200
    assert response.json()['id'] and response.json()['title'] and response.json()['submenus_count'] != [] and response.json()['dishes_count'] != []
...
def test_update_target_menu():
    ...
    response = requests.get(f'{base_url}/menus/{params.menu_id}')
    assert response.status_code == 200
    assert response.json()['title'] == params.params_menu_update['title'] and response.json()['description'] == params.params_menu_update['description']
...
def test_submenu_create():
    ...
    assert response.json()['description'] == params.params_submenu_create['description']
...
def test_submenu_get_target():
    ...
    assert response.json()['id'] != [] and response.json()['menu_id'] !=[] and response.json()['title'] != [] and response.json()['description'] != [] and response.json()['dishes_count'] != []

@pytest.mark.submenu
def test_submenu_update():
    response = requests.patch(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}', json=params.params_submenu_update)
    assert response.status_code == 200
    req = requests.get(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}')
    assert req.json()['id'] != [] and req.json()['menu_id'] !=[] and req.json()['title'] == params.params_submenu_update['title'] and req.json()['description'] == params.params_submenu_update['description'] and req.json()['dishes_count'] != []
...
def test_dish_create():
    response = requests.post(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}/dishes', json=params.params_dish_create)
    assert response.status_code == 201
    params.dish_id = response.json()['id']
    assert response.json()['id'] and response.json()['sub_menu_id'] and response.json()['menu_id']
    assert response.json()['title'] == params.params_dish_create['title']
    assert response.json()['description'] == params.params_dish_create['description']
    assert float(response.json()['price']) == float(params.params_dish_create['price'])
...
def test_dish_update():
    response = requests.patch(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}/dishes/{params.dish_id}', json=params.params_dish_update)
    assert response.status_code == 200
    req = requests.get(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}/dishes/{params.dish_id}')
    assert req.json()['title'] in params.params_dish_update['title'] and req.json()['description'] in params.params_dish_update['description'] and req.json()['price'] == params.params_dish_update['price']
...
def test_dish_delete():
    response = requests.delete(f'{base_url}/menus/{params.menu_id}/submenus/{params.submenu_id}/dishes/{params.dish_id}')
    assert response.status_code == 200
